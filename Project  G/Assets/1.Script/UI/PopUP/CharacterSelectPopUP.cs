...
public class CharacterSelectPopUP : UIPopUP
{
    ...
    [SerializeField] TextMeshProUGUI currAchievementTitle;   // 도전과제 텍스트
    ...
    private void UpdateDetailUi(CharacterType type)
    {
        ...
        // 기본 캐릭터는 도전과제 None
        if (data.AchieveType == AchieveType.None)
        {
            SetUnLockUi();
            return;
        }
        ...
        AchieveProgressResponse achieveResponse
            = GameServices.Instance.AchieveProgressModel.GetAchieveProgress(data.AchieveType);

        // 클리어했으면
        if (achieveResponse.isClear)
        {
            SetUnLockUi();
            return;
        }
        ...
        StageAchieve achieveData = AchievDataManager.Instance.GetAchieveByType(achieveResponse.AchieveType);

        SetLockUi();

        // 도전과제 타이틀 로컬라이징
        // 후 도전과제 이름 + 진행사항 표시
        cantSelectText.text = LocalizationAchieveTitle(data.AchieveType) + "\n" + UserDataManager.Instance.ReturnUserStage(achieveData.MapType)
            + " / " + achieveData.AchieveStage;
    }
